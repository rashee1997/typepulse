...
export default function TypingStudio() {
  ...
  // Handle key release for mechanical switch release acoustic feedback
  const handleKeyUp = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (sessionState !== 'playing') return; // no release clicks on an idle/finished screen
  ...
  const commandList: CommandItem[] = [
    ...
    },
    {
      ...
      subtitle: 'Architecture specs, engine details, self-hosting & FAQ',
      category: 'Views',
      icon: <BookOpen className="w-4 h-4" />,
      action: () => {
    ...
    {
      id: 'view-llms-txt',
      title: 'Open LLM Machine-Readable Index',
      subtitle: '/llms.txt specification for AI models and answer engines',
      category: 'Views',
      ...
      {isZenMode ? (
        ...
        <div className="w-full max-w-7xl mx-auto flex items-center justify-between gap-1.5 sm:gap-3 min-w-0">
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
            ...
            <Link
              href="/docs"
              className="p-1.5 sm:px-2.5 sm:py-1 rounded-xl bg-surface-muted border border-border text-text-muted hover:text-text-primary hover:bg-surface-hover transition-colors shrink-0 flex items-center gap-1.5 text-xs font-semibold cursor-pointer"
              title="Documentation, Architecture & Guides (/docs)"
